...
export class AttributeBuilder {
    ...
    addNames(...names: string[]): this {
        this.names = names.reduce((prev, name) => ({...prev, [name]: AttributeNamer.nameFor(name)}) , this.names);
        return this;
    }
    ...
    buildPath(path: string): string {
        ...
        this.addNames(...names);
        return parts.map(part => part.includes('[') ? `${this.nameFor(part.substring(0, part.indexOf('[')))}${part.substring(part.indexOf('['))}` : this.nameFor(part)).join('.');
    }
    ...
    asInput(provided?: Pick<PutItemInput, 'ExpressionAttributeNames' | 'ExpressionAttributeValues'>): Pick<PutItemInput, 'ExpressionAttributeNames' | 'ExpressionAttributeValues'> {
        ...
        const providedValues = Object.keys(provided?.ExpressionAttributeValues ?? {});
