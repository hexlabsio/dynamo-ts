...
describe('filter parser', () => {
  ...
  it('handles range expressions', () => {
    ...
    const [lowerRangeKey] = expectAttributeValueKV(
      filterParser.expressionAttributeValues,
      lowerRange,
    );
    const [upperRangeKey] = expectAttributeValueKV(
      filterParser.expressionAttributeValues,
      upperRange,
    );

    expect(filterParser.expression).toEqual(
      `#baz BETWEEN ${lowerRangeKey} AND ${upperRangeKey}`,
    );
  });
  ...
  it('handles IN expressions', () => {
    ...
    const filterParser = new Parser<Foo>({
      key: 'baz',
      comparison: ['in', [n1, n2, n3]],
    });
    expect(filterParser.expressionAttributeNames).toEqual({ '#baz': 'baz' });

    const [n1Key] = expectAttributeValueKV(
      filterParser.expressionAttributeValues,
      n1,
    );
    const [n2Key] = expectAttributeValueKV(
      filterParser.expressionAttributeValues,
      n2,
    );
    const [n3Key] = expectAttributeValueKV(
      filterParser.expressionAttributeValues,
      n3,
    );

    expect(filterParser.expression).toEqual(
      `#baz IN (${n1Key},${n2Key},${n3Key})`,
    );
  });
  ...
  it('handles begins_with expressions', () => {
    ...
    const filterParser = new Parser<Foo>({
      key: 'bop',
      comparison: ['begins_with', lookupValue],
    });
  ...
  it('handles contains expressions', () => {
    ...
    const filterParser = new Parser<Foo>({
      key: 'bop',
      comparison: ['contains', lookupValue],
    });
    expect(filterParser.expressionAttributeNames).toEqual({ '#bop': 'bop' });
  ...
  it('handles attribute_not_exists expressions', () => {
    const filterParser = new Parser<Foo>({
      key: 'bop',
      comparison: 'attribute_not_exists',
    });
    expect(filterParser.expressionAttributeNames).toEqual({ '#bop': 'bop' });
  ...
  it('handles AND expressions', () => {
    ...
    expect(filterParser.expressionAttributeNames).toEqual({
      '#bar': 'bar',
      '#baz': 'baz',
    });

    const [eqKey] = expectAttributeValueKV(
      filterParser.expressionAttributeValues,
      eqLookup,
    );
    ...
    expect(filterParser.expression).toEqual(
      `(#bar = ${eqKey}) AND (#baz BETWEEN ${lowerRangeKey} AND ${upperRangeKey})`,
    );
  });
  ...
  it('handles NOT expressions', () => {
    ...
    expect(filterParser.expression).toEqual(
      `NOT (#baz BETWEEN ${lowerRangeKey} AND ${upperRangeKey})`,
    );
  });
  ...
  it('handles AND OR NOT expressions', () => {
    ...
    const [beginsKey] = expectAttributeValueKV(
      filterParser.expressionAttributeValues,
      beginsWithLookup,
    );
    ...
    expect(filterParser.expression).toEqual(
      `((#bar = ${eqKey}) AND (NOT (#baz BETWEEN ${lowerRangeKey} AND ${upperRangeKey}))) OR (begins_with(#bop, ${beginsKey}))`,
    );
  });
});
