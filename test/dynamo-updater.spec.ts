...
const preInserts: TableType[] = [
  ...
  { hash: hash + '2', text: 'some text', obj: { abc: 'xyz', def: 2 }, mno: 2, pqr: 'yyy'  },
  { hash: hash + '3', text: 'some text', obj: { abc: 'xyz', def: 2 }, mno: 2, pqr: 'yyy'  },
...
describe('Dynamo Updater', () => {
  ...
  describe('Key conditions', () => {
    ...
    it('should act like a put when item does not exist', async () => {
      const result = await testTable.update({ key: { hash: 'another-hash' }, updates: { text: 'test', mno: 5 }, return: 'ALL_NEW' });
      expect(result.item).toEqual({ hash: 'another-hash', text: 'test', mno: 5 });
    });

    it('should remove value when undefined', async () => {
      const result = await testTable.update({ key: { hash: hash + '2' }, updates: { pqr: undefined }, return: 'ALL_NEW' });
      expect(result.item).toEqual({ ...preInserts[1], pqr: undefined });
    });

    it('should update nested value', async () => {
      const result = await testTable.update({ key: { hash: hash + '3' }, updates: { 'obj.abc': 'zyz' }, return: 'ALL_NEW' });
      expect(result.item).toEqual({ ...preInserts[2], obj: {...preInserts[2].obj, abc: 'zyz'} });
    });

    it('should increment value', async () => {
      const result = await testTable.update({ key: { hash: hash + '4' }, updates: { mno: 3 }, increments: [{ key: 'mno' }], return: 'ALL_NEW' });
      expect(result.item).toEqual({ ...preInserts[3], mno: 5 });
    });

    it('should fail condition check', async () => {
      await expect(testTable.update({
        key: { hash: hash + '4' },
        updates: { mno: 3 },
        increments: [{ key: 'mno' }],
