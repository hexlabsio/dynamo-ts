...
export function dynamoTable(definition: DynamoDefinition<any, any, any, any>, name?: string, props?: any): any {
    ...
    const keys: string[] = [...new Set([definition.hash as string, ...(definition.range ? [definition.range! as string] : []), ...indexKeys])]
    ...
    return {
        ...props,
        ...(name ? {TableName: name }: {}),
        KeySchema: [{KeyType: 'HASH', AttributeName: definition.hash as string}, ...(definition.range ? [{KeyType: 'RANGE', AttributeName: definition.range as string}]: [])],
...
export function tableDefinition(defintions: Record<string, DynamoDefinition<any, any, any, any>>): {tables: unknown[]} {
    return { tables: Object.keys(defintions).map(table => dynamoTable(defintions[table], table)) };
}

export function writeJestDynamoConfig(defintions: Record<string, DynamoDefinition<any, any, any, any>>, name = 'jest-dynamodb-config.js'): void {
