import { ConditionGroup, ConditionGroupTag, Conditions, SimpleExpression, TupleInferred, UnionType } from "src/dynamoTypes";

export class MutableConditionBuilder<T, KS extends (keyof T)[]> {
    private constructor(readonly tag: ConditionGroupTag,
    ...
    add<C extends keyof T>(s: (SimpleExpression<T, C> | MutableConditionBuilder<T, C[]>)): MutableConditionBuilder<T, KS> {
    ...
    build(): ConditionGroup<SimpleExpression<T, UnionType<KS>>> {
        switch(this.tag) {
            case 'and': return {$and: this.conditions}
            case 'or': return {$or: this.conditions}
            case 'not': return {$not: this.conditions[0]} //nope!!
        }
    }
}

export class ConditionBuilder<T, KS extends (keyof T)[]> {
    private constructor(private readonly operator: ConditionGroupTag,
    ...
    add<C extends keyof T>(s: SimpleExpression<T, C>): ConditionBuilder<T, TupleInferred<[C, ...KS]>> {
    ...
    build(): ConditionGroup<SimpleExpression<T, UnionType<KS>>> {
        switch(this.operator) {
            case 'and': return {$and: this.conditions}
            case 'or': return {$or: this.conditions}
            case 'not': return {$not: this.conditions[0]}
        }
    }
}