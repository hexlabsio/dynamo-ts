...
describe('Dynamo Querier', () => {
  beforeAll(async () => {
    ...
    await Promise.all(cars.map(async car => {
      await Promise.all(car.colours.map(async colour => {
        await Promise.all(car.years.map(async year => {
          await Promise.all(new Array(5).fill(0).map(async () => {
            const identifier = uuid();
            await tableClient.put({make: car.make, model: car.model, colour: colour, year, identifier});
          }))
        }))
      }))
    }));
  ...
  it('should query all Nissans from 2006', async () => {
    ...
    const allAre2006Nissan = !result.some(it => it.make !== 'Nissan') && !result.some(it => it.year !== 2006);
    expect(allAre2006Nissan).toEqual(true);
  });
  it('should query all Nissans beginning with 3', async () => {
    const result = await queryAllNissanModelsThatBeginWith3AndFlipOrderUsingIndex();
