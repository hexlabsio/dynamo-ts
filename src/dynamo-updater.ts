...
import {
  DynamoEntry,
  DynamoKeysFrom,
  DynamoMapDefinition, DynamoNonKeysFrom,
  DynamoRangeKey
} from "./type-mapping";
...
export type UpdateItemOptions<
    ...
    > = Partial<Pick<UpdateItemInput, 'ReturnValues' | 'ReturnConsumedCapacity' | 'ReturnItemCollectionMetrics'>> & {
...
export type UpdateReturnType<DEFINITION extends DynamoMapDefinition, RETURN_ITEMS extends ReturnValue | null> =
...
export class DynamoUpdater {
  ...
  private static updateExpression<
    ...
    function update(key: string) {
      ...
      if(inc) return `${attributeBuilder.nameFor(key)} = ` + (inc.start !== undefined ? `if_not_exists(${attributeBuilder.nameFor(key)}, ${attributeBuilder.addValue(inc.start)})`: `${attributeBuilder.nameFor(key)}`) + ` + ${attributeBuilder.addValue(key)}`
      return `${attributeBuilder.nameFor(key)} = ${attributeBuilder.addValue(props[key])}`;
    }

    const setExpression = validKeys.length > 0
        ? `SET ${validKeys.map((key) => update(key)).filter(it => !!it).join(', ')}` : undefined
    const removeExpression = removes.length > 0
        ? `REMOVE ${removes.map((key) => `${attributeBuilder.nameFor(key)} `).join(', ')}` : undefined
  ...
  > (
    ...
    const {key, updates, increments, return: returnValues, ...extras} = options;
    const updateInput: UpdateItemInput = {
      TableName: config.tableName,
      Key: key,
      UpdateExpression: this.updateExpression(attributeBuilder, updates, increments),
    ...
    const result = await config.client
        .update(updateInput)
        .promise();
    return {item: result.Attributes as any, consumedCapacity: result.ConsumedCapacity, itemCollectionMetrics: result.ItemCollectionMetrics};
  }
