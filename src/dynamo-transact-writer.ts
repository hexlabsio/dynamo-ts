import {
  ...
  TransactWriteItemsInput, Update
} from '@aws-sdk/client-dynamodb/dist-types/models';
import {TransactWriteItemsOutput,
...
import { CamelCaseKeys, DynamoConfig, DynamoFilter, JsonPath, ValueAtJsonPath } from './types';
...
> = CamelCaseKeys<
  Pick<Update, 'ReturnValuesOnConditionCheckFailure'>
> & {
...
export type TransactDeleteOptions<TableConfig extends TableDefinition> = CamelCaseKeys<
  Pick<Delete, 'ReturnValuesOnConditionCheckFailure'>
> & {
  key: TableConfig['keys'],
  condition?: DynamoFilter<TableConfig['type']>;
};

export type TransactConditionOptions<TableConfig extends TableDefinition> = CamelCaseKeys<
  Pick<ConditionCheck, 'ReturnValuesOnConditionCheckFailure'>
> & {
  key: TableConfig['keys'],
  condition: DynamoFilter<TableConfig['type']>;
};
...
export class TransactWriteExecutorHolder implements TransactWriteExecutor
{
  ...
  async execute(options: TransactWriteOptions): Promise<TransactWriteReturn> {
    return await new TransactWriteClient(this.client, [this], this.logStatements).execute(options);
  }
  ...
  then<B extends TransactWriteExecutor>(
    ...
    return new TransactWriteClient(this.client, [this, other], this.logStatements);
  }
}
...
export class TransactWriteClient<T extends TransactWriteExecutor[]> {
  ...
  then<B extends TransactWriteExecutor>(
    ...
    return new TransactWriteClient<[...T, B]>(this.client, [
      ...this.executors,
      other,
    ], this.logStatements);
  }
  ...
  async execute(options: TransactWriteOptions = {}): Promise<TransactWriteReturn> {
    ...
    if (this.logStatements) {
      console.log(`TransactWriteCommandInput: ${JSON.stringify(input, null, 2)}`);
    }
...
export class DynamoTransactWriter<TableConfig extends TableDefinition> {
  ...
  put(options: TransactPutOptions<TableConfig['type']>): TransactWriteExecutor {
    ...
    return new TransactWriteExecutorHolder(client, {TransactItems: [{Put: put}]}, this.clientConfig.logStatements);
  }
  ...
  delete(options: TransactDeleteOptions<TableConfig>): TransactWriteExecutor {
    ...
    const client = this.clientConfig.client;
    return new TransactWriteExecutorHolder(client, {TransactItems: [{Delete: deleteInput}]}, this.clientConfig.logStatements);
  }

  conditionCheck(options: TransactConditionOptions<TableConfig>): TransactWriteExecutor {
    const attributeBuilder = AttributeBuilder.create();
    const condition =
      options.condition &&
      filterParts(attributeBuilder, options.condition);
    ...
    const client = this.clientConfig.client;
    return new TransactWriteExecutorHolder(client, {TransactItems: [{ConditionCheck: conditionCheck}]}, this.clientConfig.logStatements);
  }
  ...
  update<KEY extends JsonPath<TableConfig['type']>>(
    ...
    const client = this.clientConfig.client;
    return new TransactWriteExecutorHolder(client, {TransactItems: [{Update: update}]}, this.clientConfig.logStatements);
  }
