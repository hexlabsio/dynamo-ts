...
describe('Dynamo Getter', () => {
  ...
  describe('Single Table', () => {
    it('should batch get single table', async () => {
      const executor = testTable.batchGetExecutor([{identifier: '0'}, {identifier: '3'}, {identifier: '4'}]);
      console.log(JSON.stringify(executor.input, null, 2));
      const result = await executor.execute();
      expect(result.items).toEqual([{identifier: '0', text: '0'}, {identifier: '3', text: '3'}, {identifier: '4', text: '4'}]);
    });
  });
  ...
  describe('Multi Table', () => {
    it('should batch get multi table', async () => {
      const result = await
        testTable.batchGetExecutor(
          [{identifier: '0'}, {identifier: '3'}, {identifier: '4'}]
        )
        .and(
          testTable2.batchGetExecutor(
            [{identifier: '10000', sort: '0'}, {identifier: '10008', sort: '8'} ],
            { projection: projector => projector.project('sort')}
          )
        )
        .execute();
      expect(result.items).toEqual([
        [{identifier: '0', text: '0'}, {identifier: '3', text: '3'}, {identifier: '4', text: '4'}],
        [{sort: '8'}, {sort: '0'}]
      ]);
    });
  });
