...
const preInserts: TableType[] = [
  ...
  { hash: 'delete-item-test-2', text: 'some text', obj: { abc: 'xyz', def: 2 } },
  { hash: 'delete-item-test-3', text: 'some text', obj: { abc: 'xyz', def: 2 } },
...
describe('Dynamo Deleter', () => {
  ...
  describe('Delete', () => {
    ...
    it('should delete item', async () => {
      ...
      const before = await dynamoClient.get({TableName, Key: {hash}}).promise();
      expect((before.Item as any).hash).toEqual(hash);
      await testTable.delete({ hash });
      const after = await dynamoClient.get({TableName, Key: {hash}, ConsistentRead: true}).promise();
    ...
    it('should delete item and return old value', async () => {
      ...
      const before = await dynamoClient.get({TableName, Key: {hash}}).promise();
      expect((before.Item as any).hash).toEqual(hash);
      const deleted = await testTable.delete({ hash }, { returnValues: 'ALL_OLD' });
      const after = await dynamoClient.get({TableName, Key: {hash}, ConsistentRead: true}).promise();
      expect(after.Item).toBeUndefined();
    ...
    it('should throw when condition not met', async () => {
      ...
      expect((before.Item as any).hash).toEqual(hash);
      await expect(
        testTable.delete({ hash }, { condition: compare => compare().text.eq('not this') })
      ).rejects.toThrow('The conditional request failed');
    });

    it('should succeed when deleting item that does not exist', async () => {
      const hash = 'delete-item-test-not-present';
      const result = await testTable.delete({ hash }, { returnValues: 'ALL_OLD' });
    ...
    it('should delete item when condition met', async () => {
      ...
      const before = await dynamoClient.get({TableName, Key: {hash}}).promise();
      expect((before.Item as any).hash).toEqual(hash);
      await testTable.delete({ hash }, { condition: compare => compare().text.eq('some text') })
      const after = await dynamoClient.get({TableName, Key: {hash}, ConsistentRead: true}).promise();
      expect(after.Item).toBeUndefined();
    });
    ...
    it('should return consumed capacity', async () => {
      ...
      const result = await testTable.delete({ hash }, { returnConsumedCapacity: "TOTAL" })
